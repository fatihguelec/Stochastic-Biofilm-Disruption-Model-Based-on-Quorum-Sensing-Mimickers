...
% This version is only to validate the experimental bacterial growth
...
t_s = 16; % Total simulation time (h)
...
k_c = k_g/((1+Y_BS)*K_M);
r_g = k_g.*[1.5 1]; % Bacterial growth rate constant (h^-1)
r_c = k_c*C_glu.*[0.1 1];  % Rate constant of complex formation (h^-1) 
r_dm = 0.001*r_sigma; % Degradation rate of M (h^-1)

% State S1
r_a_1 = 3.8*10^-6*N_av*V/U; % Production rate of A (h^-1) at S1 (Henkel et al., 2013)
% mass_eps = 1510*1.66054e-24; % Molecular mass of EPS (g) (Banerjee et al., 2021)
% r_e_1 = 0.84*mu_max/(24*mass_eps*U); % Production rate of E at S1 (Frederick et al., 2011)
...
% r_e_2 = 8.4*mu_max/(24*mass_eps*U); % Production rate of E at S2 (Frederick et al., 2011)
...
C_B_t_mean = zeros(length(r_g),length(t_vec));

for i_rgc = 1:length(r_g)
    ...
    for i_mc = 1:MC
        ...
        while t <= t_s
            ...
            a(7) = r_c(i_rgc)*X(2)*X(5); % Reaction 7 - Production of C
            a(8) = r_g(i_rgc)*X(6); % Reaction 8 - Production of B
    ...
    %         % Modified explicit Tau-Leap Algorithm
    %         [t, X] = tau_leap_expl(t, a, X, nu, V, N_reac, gamma_QS, gamma_DE, gamma_DB, r_a_1, r_a_2, r_e_1, r_e_2, r_e_d, r_d, r_sigma, r_c, r_g);
...
B_growth_exp_table = readtable('bacterial_growth_5c_LB_medium.xlsx');
B_growth_exp = table2array(B_growth_exp_table);
min_norm = min(B_growth_exp(:,2));
max_norm = max(B_growth_exp(:,2));
C_B_t_mean_norm(1,:) = normalize(C_B_t_mean(1,:),'range', [min_norm max_norm]);
C_B_t_mean_norm(2,:) = normalize(C_B_t_mean(2,:),'range', [min_norm max_norm]);


% Normalized together
h_f = figure;
semilogy(t_vec2(2:end), B_growth_exp(:,2), 'b--*', t_vec, C_B_t_mean_norm(1,:), 'r-', t_vec, C_B_t_mean_norm(2,:), 'r--','LineWidth', 1.25);
xlabel('Time (h)'); ylabel('Normalized bacterial concentration (CFU/l)');
legend('Experiment (in vitro)', ['Simulation (r_g, r_c)=(', num2str(r_g(1)),', ' num2str(r_c(1)), ')'], ['Simulation (r_g, r_c)=(', num2str(r_g(2)),', ' num2str(r_c(2)), ')']); 
grid on;
xlim([0 17]); 
ylim([0.1 4]);
...
% print(h_f,sprintf('Val_5c_exp_sim_MC_1000.pdf'),'-dpdf','-r0') %save as pdf 
toc
